...
const skipSeen = through2.obj(function(data, enc, callback) {
  ...
  const cnetHash = data.hash ? data.hash : hash(body, { excludeKeys: function(key) {
        if ( key === 'source' || key === 'sourceRun' || key === 'date' ) {
          return true;
        }
        return false;
      }
  ...
  collection.count().then(count => {
    ...
    collection
      ...
      .then(doc => {
          // console.log(isNull(doc), typeof cnetHash);
        if (isNull(doc) && hashCache[cnetHash] === undefined) {
            hashCache[cnetHash] = 1;
          self.push(data);
        } else {
          skipped++;
        }
  ...
  return stream
    ...
    .pipe(
      etl.map(data => {
        ...
        const cnetHash = data.hash ? data.hash : hash(oldbody, { excludeKeys: function(key) {
              if ( key === 'dataSource' || key === 'date' ) {
                return true;
              }
              return false;
            }
